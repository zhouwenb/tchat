...
def reply_pubsub_message(r, *args, **kwargs):
    ...
    '''
    if len(r) == 3:
        (kind, channel, body) = map(lambda x:isinstance(x, bytes) and x.decode('utf-8'), r)
        pattern = channel
    elif len(r) == 4:
        (kind, pattern, channel, body) = map(lambda x:isinstance(x, bytes) and x.decode('utf-8'), r)
    elif len(r) == 1:
        kind = isinstance(r[0], bytes) and r[0].decode('utf-8')
...
#                 'password', 'selected_db', '_pipeline', '_weak')
    ...
    def psubscribe(self, channels):
        return self._subscribe('PSUBSCRIBE', channels)

    def _subscribe(self, cmd, channels):
        if isinstance(channels, str):
            channels = [channels]
        result = self.exe_command(cmd, *channels)
        if result:
            self.subscribed = True
        return result

    def unsubscribe(self, channels):
        self._unsubscribe('UNSUBSCRIBE', channels)

    def punsubscribe(self, channels):
        self._unsubscribe('PUNSUBSCRIBE', channels)

    def _unsubscribe(self, cmd, channels):
        if isinstance(channels, str):
            channels = [channels]
        result = self.exe_command(cmd, *channels)
        if result:
